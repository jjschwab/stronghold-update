...
export default function Projects() {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("switchProject", handleProjectSwitch);
    };
  }, []);
  ...
  useEffect(() => {
    // KEN BURNS TUNING: Increased timer to 6s (6000ms) to allow the slow pan to breathe
  ...
  return (
    <section id="projects" className="relative w-full pb-32 pt-0 px-4 overflow-hidden scroll-mt-28">
      ...
      <style jsx global>{`
        ...
        /* Slow, deliberate movement over 20s. The image swap happens at 6s, 
           so we only see the start of the curve, keeping it subtle. */
        .animate-ken-burns {
              ...
              <div className="relative flex-1 p-0 flex flex-col justify-end min-h-[400px]">
                ...
                <div className="absolute inset-0 z-0 overflow-hidden bg-gray-900">
                    {activeProject.images.map((img, index) => (
                        ...
                        >
                            {/* KEN BURNS IMAGE
                                - object-cover: Fills the space (Full Bleed)
                                - object-top: Anchors to top (showing headers/nav)
                                - animate-ken-burns: The slow zoom/pan
                            */}
                            <img 
                                src={img} 
                            ...
                            {/* GRADIENT OVERLAY - Stronger gradient to ensure text readability against full-bleed UI */}
                ...
                {/* CONTENT LAYER */}
                <div className="relative z-30 pointer-events-none p-8 md:p-12">
                  ...
                  <div className="flex gap-8 mb-8">
                    {activeProject.stats.map((stat, i) => (
                      <div key={i} className="flex flex-col">
                        ...
                        <span className={`${inter.className} text-white text-sm md:text-base border-l-2 border-white/20 pl-3`}>
                          {stat.value}
                        </span>
                      </div>
                    ))}
                  ...
                  <p className="text-gray-300 leading-relaxed text-lg md:text-xl max-w-2xl drop-shadow-md">
                  ...
                  <div className="flex gap-2 mt-6">
                    {activeProject.images.map((_, idx) => (
                        <div 
                            key={idx}
                            className={`h-1 rounded-full transition-all duration-300 ${idx === currentImgIndex ? "w-8 bg-white" : "w-2 bg-white/20"}`}
                        ></div>
                    ))}
                  </div>
            ...
            {projects.map((project) => {
              ...
              return (
                <button
                  ...
                  className={`
                    group flex items-center h-20 border transition-all duration-500 overflow-hidden relative backdrop-blur-sm
                    ${isActive 
                      ? "bg-white text-black border-white" 
                      : "bg-white/5 text-gray-500 border-white/10 hover:border-white/30 hover:bg-white/10 hover:text-white"}
                  `}
                >
                  
                  <div className={`
                    flex items-center justify-center h-full transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] whitespace-nowrap
                    w-full group-hover:w-1/2
                  `}>
                  ...
                  <div className={`
                    flex items-center justify-center h-full overflow-hidden bg-white/5 
                    transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)]
                    w-0 opacity-0 group-hover:w-1/2 group-hover:opacity-100
                    ${isActive ? "border-l border-black/10 text-black/60" : "border-l border-white/10 text-gray-400"}
                  `}>
